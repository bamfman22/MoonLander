...
//
void Octree::drawBox(const Box &box, int color) {
...
void Octree::create(const ofMesh & geo, int numLevels) {
    ...
    vector<int> indicesToInt(indices.size());
    vector<int> points;
    
    for(int i = 0; i < indices.size(); i++){
        indicesToInt[i] = indices[i];
    }
    
    int totalPoints = getMeshPointsInBox(geo, indicesToInt, boundingBox, points);
    ...
    cout << end - start << "Milliseconds to create the octree" << endl << endl;
